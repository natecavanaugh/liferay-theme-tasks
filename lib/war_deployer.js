...
WarDeployer.prototype = _.create(EventEmitter.prototype, {
	...
	_onResponseEnd: function() {
		if (this.deployed) {
			log(colors.cyan(this.fileName + '.war'), 'successfully deployed to', this.host);
		}
		else {
			log(colors.yellow('Warning:'), 'There was a problem deploying', colors.cyan(this.fileName + '.war'), 'to', this.host);
		}
	},
